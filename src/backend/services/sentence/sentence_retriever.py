...
class SentenceRetriever:
    def __init__(self, user_proficiency: str = "intermediate"):
        """
    ...
    def load_existing_indexes(self):
        ...
        with Session(self.engine) as session:
            for language in languages:
                max_id: int = session.exec(
                    select(func.max(Phrase.id)).where(Phrase.language == language)
                ).one()
                languages_max_id.append(max_id)

        for language, max_id in zip(languages, languages_max_id):
            index_path = INDEX_DIR / f"sentence_index_{language}_{max_id}.pkl"
    ...
    def add_phrase_to_index(self, text: Text, phrase: Phrase,  language: str):
        ...
        index_data['tokenized_texts'].append(tokens)
